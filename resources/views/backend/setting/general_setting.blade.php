<form action="{{ route('admin.general.basic') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="mb-4 col-xxl-12 col-lg-12 col-sm-12">
            <label for="exclusive_offer_date">{{ __('Exclusive Offer Expiry Date') }}</label>
            <input type="date" name="exclusive_offer_date" class="form-control" value="{{ Config::config()->exclusive_offer_date?->format('Y-m-d') ?? '' }}">
